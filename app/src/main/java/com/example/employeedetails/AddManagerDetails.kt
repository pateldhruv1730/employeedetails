...
import kotlinx.android.synthetic.main.activity_add_manager_details.*

class AddManagerDetails : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_add_manager_details)
        ...
        var rs: Cursor = db.rawQuery("SELECT * FROM MANAGERINFO WHERE USERID=?", check)
        rs.requery()
        if (rs.moveToNext()) {
        ...
        btnstudent.setOnClickListener {
            var intent = Intent(this, AddEmployeeInfo::class.java)
            intent.putExtra("USER", userid)
            startActivity(intent)
        }
    }
